...
async def create_user(tg_id: int, session: AsyncSession, user_data: dict):
    user = User(
        ...
        activity_level=user_data[CacheKeys.UserData.activity()],
    ...
    user_calories = UserCalories(
        maximum_quantity=user_data[CacheKeys.Calories.maximum_quantity()],
        user_id=user.telegram_id
    )
...
async def get_user_data(session: AsyncSession, user_id: int) -> dict:
    ...
    return {
        ...
        CacheKeys.UserData.activity(): user.activity_level,
        ...
        CacheKeys.Calories.maximum_quantity(): user.calories.maximum_quantity,
        CacheKeys.Calories.current_quantity(): user.calories.current_quantity
    }
